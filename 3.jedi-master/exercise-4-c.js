...
function createList(infos) {
  ...
  infos.forEach(({ title, imgUrl }, index) => {
    itemListComponent +=
      `<div id=${index + 1} class="list-item">
        <h4>${title}</h4>
        <img src=${imgUrl} alt=${title}>
        <button>Remove this element</button>
      </div>
      `
  });
...
function addButtonClickEvent() {
  document.querySelectorAll("button").forEach((btn, index) => {
    if (btn.textContent === "Remove last element") {
      btn.addEventListener("click", () => removeLastListItem())
      return
    }
    removeCurrentItem(btn, index)
  })
}
...
function removeCurrentItem(btn, id) {
  const itemToBeRemoved = document.getElementById(id)
  btn.addEventListener("click", () => itemToBeRemoved.remove())
}
